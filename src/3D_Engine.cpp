...
/*    ===================== */
static long LockAngle(long opposite, long adjacent, long clockwise);
...
void GetSinCos(long angle, short* sin, short* cos) {
    *sin = Sin_Cos[angle];
...
void CalcYXZTrigCoefficients(long x_angle, long y_angle, long z_angle) {
...
short* TrigCoefficients(void) { return (Trig_Coeffs); }
...
void WorldOffset(long* xptr, long* yptr, long* zptr) {
    long x, y, z;
    ...
    *xptr = (x * static_cast<long>(Trig_Coeffs[X_X_COMP])) + (y * static_cast<long>(Trig_Coeffs[Y_X_COMP])) +
            (z * static_cast<long>(Trig_Coeffs[Z_X_COMP]));

    *yptr = (x * static_cast<long>(Trig_Coeffs[X_Y_COMP])) + (y * static_cast<long>(Trig_Coeffs[Y_Y_COMP])) +
...
long TransformedZ(long offset) { return (Transformed_Coords[offset].z); }
...
{                                // in the order: origin, u, v
    ...
    SetTextureVectors(ox, oy, oz, ux, uy, uz, vx, vy, vz);
...
long PolygonVisible(long* cptr) // pointer to co-ordinate offsets for polygon
{
    ...
    offset = *cptr++;
    x1 = Screen_Coords[offset].x;
    y1 = Screen_Coords[offset].y;
    offset = *cptr++;
    x2 = Screen_Coords[offset].x;
    y2 = Screen_Coords[offset].y;
    offset = *cptr;
    x3 = Screen_Coords[offset].x;
    y3 = Screen_Coords[offset].y;
          ...
          long c2) // co-ordinate offset for line, point 2
{
    ...
    TLVertices[0].tu = 0.0f;
    TLVertices[0].tv = 1.0f;
...
long PolygonZClipped(long* cptr, // pointer to co-ordinate offsets for polygon
                     long sides, long check_orientation,
                  ...
                  long c2) // co-ordinate offset for line, point 2
{
    ...
    if (point1_below) {
        ...
        ZClip(point1, point2, // below, above
              screen_width, screen_height, &x1, &y1);
    } else {
        // store current screen co-ordinate (because it is above boundary)
        x1 = Screen_Coords[c1].x;
        y1 = Screen_Coords[c1].y;
    ...
    if (point2_below) {
        ...
        ZClip(point2, point1, // below, above
              screen_width, screen_height, &x2, &y2);
    } else {
        // store current screen co-ordinate (because it is above boundary)
        x2 = Screen_Coords[c2].x;
        y2 = Screen_Coords[c2].y;
...
void LockViewpointToTarget(long viewpoint_x, long viewpoint_y, long viewpoint_z, long target_x, long target_y,
